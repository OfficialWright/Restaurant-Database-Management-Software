...
public class tableSelect extends JFrame {
	tablePanelBuilder updatePanel;
	tablePanelBuilder searchPanel;
	JPanel buttonPanel;
	...
	public tableSelect() {
		...
		setTitle("Select/Update/Delete Table");
  
		// Specify an action for the close button
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
  
		// Create a panelBuilder object
		updatePanel = new tablePanelBuilder("Update Table");
		searchPanel = new tablePanelBuilder("Search Table");
	...
	private void buildListPanel() {
		try {
			...
			createTitledBorder("Table Information"));
			...
			ResultSet info = tableManager.selectTable("", "");
						...
						// Check to make sure table is not being reloaded
							if (ourTable.getSelectedRow() >= 0) {
								...
								try {
									...
									updateInfo = tableManager.selectUpdateTable(selectedValue);

									while (updateInfo.next()) {
										updatePanel.setName(updateInfo.getString("Name"));
										updatePanel.setLoc(updateInfo.getString("Location"));
									}
	...
	private class SubmitButtonListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			String name = updatePanel.getName();
			String location = updatePanel.getLoc();
			...
			try {
				...
				updater.updateTable(name, location, selValue);
			...
			try {
				...
				tableManager getInfo = new tableManager();
				// This creates a blank search of the table
				searchInfo = getInfo.selectTable("", "");
			} catch (SQLException e1) {
				e1.printStackTrace();
			}
	...
	private class SearchButtonListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			updatePanel.clear();
			// Get the information from the text fields
			String name = searchPanel.getName();
			String location = searchPanel.getLoc();
			...
			try {
				...
				tableManager getInfo = new tableManager();
				// This creates a blank search of the table
				searchInfo = tableManager.selectTable(name, location);
			} catch (SQLException e1) {
				e1.printStackTrace();
			}
	...
	private class ClearButtonListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				tableManager getInfo = new tableManager();
				//This creates a blank search of the table
				searchInfo = tableManager.selectTable("", "");
			} catch (SQLException e1) {
				e1.printStackTrace();
			}
			// Resets the table to the search criteria given above
